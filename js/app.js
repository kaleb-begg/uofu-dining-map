...
function matchesSearch(d, search) {
  ...
  return `${d.name} ${d.building} ${d.type}`.toLowerCase().includes(search);
}
...
function markerRadius(d, rows) { return 8 + normalize(valueFor(d), activeMetric, rows) * 13; }
function colorRadius() { return 9; }
...
function draw() {
  ...
  rows.forEach(d => {
    ...
    const m = L.circleMarker([d.lat, d.lng], {
      ...
      fillOpacity: displayMode === 'color' ? 0.94 : 0.86
    }).bindPopup(popupHtml(d)).on('click', () => selectLocation(d.id));
    markerLayer.addLayer(m);
    markers.push(m);
    markerById[d.id] = m;
  });
  updateSearchSuggestions(rows);
...
function populateTypeFilter() {
  ...
  wrap.querySelectorAll('button').forEach(btn => btn.addEventListener('click', () => {
    ...
    document.querySelectorAll('#typeButtons .type-chip').forEach(b => {
...
function updateSearchSuggestions(rows=filteredLocations()) {
  const box = document.getElementById('searchSuggestions');
  if (!box) return;
  const search = filterText();
  if (!search) { box.innerHTML = '<span class="hint">Start typing to see matching locations.</span>'; return; }
  const matches = locations
    .filter(d => matchesSearch(d, search) && (selectedType === 'all' || d.type === selectedType))
    .slice(0, 8);
  if (!matches.length) { box.innerHTML = '<span class="hint">No matches. Try a building like Union, Gardner, or Kahlert.</span>'; return; }
  box.innerHTML = matches.map(d => `<button type="button" class="suggestion" onclick="chooseSuggestion('${escapeAttr(d.id)}')"><b>${escapeHtml(d.name)}</b><span>${escapeHtml(d.building)} · ${escapeHtml(d.type)}</span></button>`).join('');
}
